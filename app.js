(function() {
  ...
  const popoverAccessibleLabel = document.getElementById('popoverAccessibleLabel');
  ...
  function scheduleFitWhenReady() {
    if (!mapContainer || !svgDoc || mapScreen.classList.contains('hidden')) return;
    clearTimeout(resizeDebounce);
    resizeDebounce = setTimeout(() => {
      const rect = mapContainer.getBoundingClientRect();
      if (rect.width >= MIN_CONTAINER_SIZE && rect.height >= MIN_CONTAINER_SIZE) fitMapToView();
    }, 50);
  }
  ...
  function setMode(newMode) {
    ...
    mode = newMode;
    toolButtons.forEach(btn => btn.classList.toggle('active', btn.dataset.mode === newMode));
    hidePopover();
    clearHighlights();
  ...
  function onWheel(e) {
    if (e.ctrlKey || e.metaKey) {
      ...
      e.preventDefault();
      const rect = mapContainer.getBoundingClientRect();
      const viewW = getViewW(), viewH = getViewH();
  ...
  function onPanStart(e) {
    if (e.target.closest('rect[data-sec]') || e.target.closest('[data-section-id]') || e.target.closest('.zoom-controls')) return;
    isPanning = true;
  ...
  function zoomAtPoint(containerX, containerY, factor) {
    ...
    const newScale = Math.max(referenceScale * MIN_ZOOM, Math.min(referenceScale * MAX_ZOOM, scale * factor));
  ...
  function getViewW() { return vb.w / scale; }
  function getViewH() { return vb.h / scale; }
  ...
  function zoomAtCenter(factor) {
    const rect = (svgTransformWrap && svgTransformWrap.getBoundingClientRect().width > 0)
  ...
  function zoomToSection(sectionId) {
    ...
    let newScale = Math.max(referenceScale * MIN_ZOOM, Math.min(referenceScale * MAX_ZOOM, Math.max(referenceScale, s)));
  ...
  function findNzSections(svg) {
    ...
    const walk = (el) => {
      ...
      const cls = String((el.className && (el.className.baseVal || el.className)) || '').trim();
      ...
      if (hasNz && secMatch) {
        ...
        const hasSeats = el.querySelector && el.querySelector('rect[class*="-row-"][class*="-seat-"]');
        if (!hasSeats) nzSectionIds.add(secId);
      }
      Array.from(el.children || []).forEach(walk);
    };
    walk(svg);
    return Array.from(nzSectionIds).sort((a, b) => String(a).localeCompare(b, undefined, { numeric: true }));
  }

  function showNzCapacityModal(nzSections) {
    nzInputs.innerHTML = nzSections.map(sec =>
  ...
  function handleNzSubmit(e) {
    e.preventDefault();
    if (!nzForm) return;
    const nzSectionIds = Array.from(nzInputs.querySelectorAll('input[name]')).map(inp => inp.name);
  ...
  function loadSvg(svgString) {
    ...
    seats.forEach(rect => {
      ...
      if (m) {
        ...
        const isDA = secU === 'DA' || rowU === 'DA' || /da|accessible/i.test(cls);
    ...
    function addWheelchairIcons() {
      ...
      accessibleRects.forEach(rect => {
        ...
        g.setAttribute('transform', `translate(${cx},${cy}) scale(${scale}) translate(${-WHEELCHAIR_CENTER},${-WHEELCHAIR_CENTER})`);
    ...
    identifierElements = [];
    sectionPaths = [];
    const sectionIds = new Set([...Object.keys(seatData.sections), 'mf1', 'mf2', 'MF1', 'MF2']);
    const sectionPattern = /^(?:mf[12]|\d+)$/i;
    const seatRects = svgDoc.querySelectorAll('rect[data-sec]');
    svgDoc.querySelectorAll('path[class], polygon[class], rect[class], g[class]').forEach(el => {
      ...
      const isSectionCover = (hasSecMatch || tokens.some(t => sectionIds.has(t) || sectionPattern.test(t))) && !cls.includes('-seat-') && !cls.includes('-row-');
      ...
      const fillRaw = ((el.getAttribute && el.getAttribute('fill')) || '').trim().toLowerCase();
      const isTransparentFill = !fillRaw || fillRaw === 'none' || fillRaw === 'transparent';
      const isWhiteFill = fillRaw === '#fff' || fillRaw === '#ffffff' || fillRaw === 'white';
      const hasVisibleFill = !isTransparentFill && !isWhiteFill;
      const isSeparatorLine = (isTransparentFill || isWhiteFill) && stroke && stroke !== 'none';
      if (isSeparatorLine) return;
      identifierElements.push(el);
      let secId = tokens.find(t => sectionIds.has(t) || sectionPattern.test(t));
      if (!secId) {
      ...
      if (secId && sectionIds.has(secId)) {
        ...
        el.addEventListener('click', (e) => { e.stopPropagation(); zoomToSection(secId); });
        el.addEventListener('mouseenter', (e) => onSectionPathEnter(e, secId));
    ...
    gaSectionIds.forEach(gaSecId => {
      ...
      gaGroups.forEach(g => {
        const cls = String((g.className && (g.className.baseVal || g.className)) || '');
        ...
        g.querySelectorAll('path, polygon, rect, g').forEach(desc => {
          ...
          desc.addEventListener('click', (e) => { e.stopPropagation(); zoomToSection(gaSecId); });
          desc.addEventListener('mouseenter', (e) => onSectionPathEnter(e, gaSecId));
          desc.addEventListener('mouseleave', onSectionPathLeave);
        });
      });
    });

    if (nzSections.length > 0) showNzCapacityModal(nzSections);
  ...
  function fitMapToView() {
    ...
    const rect = (svgTransformWrap && svgTransformWrap.getBoundingClientRect().width > 0)
      ? svgTransformWrap.getBoundingClientRect()
      : mapContainer.getBoundingClientRect();
    if (rect.width < MIN_CONTAINER_SIZE || rect.height < MIN_CONTAINER_SIZE) return;
    ...
    referenceScale = fitScale * Math.pow(2, ((INITIAL_ZOOM_PCT - 100) / 100) * ZOOM_EXP);
    const initialScale = referenceScale;
    scale = Math.max(referenceScale * MIN_ZOOM, Math.min(referenceScale * MAX_ZOOM, initialScale));
    viewBoxX = scale >= 1 ? 0 : vb.w * (1 - 1 / scale) / 2;
    viewBoxY = scale >= 1 ? 0 : vb.h * (1 - 1 / scale) / 2;

    if (animateInitialFit) {
      animateInitialFit = false;
      const targetScale = Math.max(referenceScale * MIN_ZOOM, Math.min(referenceScale * MAX_ZOOM, initialScale));
      const targetVbx = targetScale >= 1 ? 0 : vb.w * (1 - 1 / targetScale) / 2;
      const targetVby = targetScale >= 1 ? 0 : vb.h * (1 - 1 / targetScale) / 2;
  ...
  function renderPanel() {
    ...
    const sorted = Object.entries(seatData.sections).sort((a, b) => String(a[0]).localeCompare(b[0], undefined, { numeric: true }));
    sectionsListEl.innerHTML = sorted.map(([sec, count]) =>
  ...
  function applyIdentifierStyle() {
    identifierElements.forEach(el => {
      ...
      const ancestorSecId = ancestorWithSec && ancestorWithSec !== el ? ancestorWithSec.dataset.sectionId : null;
      const effectiveSecId = (ancestorSecId && gaSectionIds.has(ancestorSecId)) ? ancestorSecId : (secId || ancestorSecId);
      const isGA = (effectiveSecId && gaSectionIds.has(effectiveSecId)) || isInsideNzSection(el);
  ...
  function onSeatEnter(e) {
    const rect = e.target;
    const sec = rect.dataset.sec, row = rect.dataset.row, seat = rect.dataset.seat;
    ...
    if (mode === 'seat') {
      ...
      showPopover(rect, { sec, row, seat, type: 'seat', accessible, accessibleType });
    } else if (mode === 'row') {
      highlightRow(sec, row);
      const count = seatData.bySecRow[sec]?.[row] || 0;
      const accessible = rect.dataset.accessible === 'true';
      const accessibleType = rect.dataset.accessibleType || null;
      showPopover(rect, { sec, row, rowSeats: count, type: 'row', accessible, accessibleType });
      ...
      showPopover(rect, { sec, sectionSeats: totalSeats, sectionRows: totalRows, type: 'section', accessible, accessibleType });
    }
  }

  function onSeatLeave(e) {
    if (e.relatedTarget && (e.relatedTarget.closest('.seat-highlight') || e.relatedTarget.closest('[data-sec]') || e.relatedTarget.closest('[data-section-id]'))) return;
    hidePopover();
    clearHighlights();
  }
  ...
  function onSectionPathEnter(e, sectionId) {
    ...
    const accessible = secU === 'DA' || secU === 'DB';
    const accessibleType = secU === 'DA' ? 'DA' : (secU === 'DB' ? 'DB' : null);
    showPopoverAt(e.clientX, e.clientY, { sec, sectionSeats: totalSeats, sectionRows: totalRows, type: 'section', isGA, accessible, accessibleType });
  }

  function onSectionPathLeave(e) {
    if (e.relatedTarget && (e.relatedTarget.closest('[data-section-id]') || e.relatedTarget.closest('rect[data-sec]'))) return;
    hidePopover();
    clearHighlights();
  }
  ...
  const SECTION_DISPLAY_WORDS = [
    'CENTER', 'SUITE', 'FULTON', 'PRESS', 'BOX', 'CLUB', 'PATIO', 'PARTY'
  ...
  function formatSectionDisplayName(secId) {
    ...
    let s = String(secId).trim().replace(/\s*NZ\s*$/i, '').trim();
    if (!s) return '';
  ...
  function highlightRow(sec, row) {
    clearHighlights();
    svgDoc.querySelectorAll(`rect[data-sec="${sec}"][data-row="${row}"]`).forEach(r => r.classList.add('seat-highlight'));
  }

  function highlightSection(sec) {
    clearHighlights();
    svgDoc.querySelectorAll(`rect[data-sec="${sec}"]`).forEach(r => r.classList.add('seat-highlight'));
  }

  function clearHighlights() {
    svgDoc?.querySelectorAll('.seat-highlight').forEach(r => r.classList.remove('seat-highlight'));
  }
  ...
  function showPopover(anchor, data) {
    ...
    if (popoverAccessibleLabel && accessible) {
      popoverAccessibleLabel.textContent = getAccessibleLabel(data.accessibleType);
    }
    if (data.type === 'seat') {
